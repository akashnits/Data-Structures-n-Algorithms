...
{ 
    ...
    { 
        ...
        nodes= new Node[n]; 
        for(int i=0; i<n; i++){ 
            nodes[i]= new is_bst_hard().new Node(scan.nextLong(), scan.nextInt(), scan.nextInt()); 
        } 
        Node root = nodes[0]; 
    ...
    public static void inOrderTraversal(Node node){ 
        ...
        if(node.getKey() < max || ((node.getKey() == max) && (hasLeftChild(node)))){ 
            flag = false; 
            return; 
        }else{ 
            max= node.getKey(); 
